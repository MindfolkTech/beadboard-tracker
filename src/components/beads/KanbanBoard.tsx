    ...
    <div className="flex flex-col gap-4 h-full p-6">
      ...
      <div className="flex flex-col">
        ...
        <div className="flex gap-3 p-3 bg-[hsl(var(--surface-accent))] rounded-b-lg overflow-x-auto">
          {doneColumn.issues.length === 0 ? (
            <div className="flex items-center justify-center w-full h-32 text-text-muted text-sm">
              No completed issues
            </div>
          ) : (
            doneColumn.issues.map(issue => (
              <div key={issue.id} className="w-[280px] shrink-0">
                <KanbanCard
                  issue={issue}
                  allIssues={issues}
                  isSelected={selectedIssueId === issue.id}
